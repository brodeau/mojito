...
for NEMO_EXP in ${LIST_NEMO_EXP}; do
    ...
    for fnc in ${list_seed_nc}; do
        ...
        if [ "${LIST_RD_SS}" = "" ]; then
            ...
            CMD="${EXE} -i ${FSI3IN} -m ${FNMM} -s ${fnc} -N ${NEMO_CONF} -R ${RESKM}km" ; # with nc file for init seed...
            ...
            for rdss in ${LIST_RD_SS}; do
                ...
                #
                if [ "${fnd}" != "" ] && [ -f ${fnd} ]; then
                    CMD="${EXE} -i ${FSI3IN} -m ${FNMM} -s ${fnd} -N ${NEMO_CONF} -R ${rdss}-${RESKM}km" ; # with nc file for init seed...
                    echo; echo " *** About to launch:"; echo "     ${CMD}"; echo
                    #exit;#lolo
