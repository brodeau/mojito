...
# CALL: recycleQuads(  QUADS, vPQids,    )
def recycleQuads( pCoor, pQuads, pPQids ):
    '''
          pQuads:  Quad object at the previous record...

          returns all arrays necessary to define a new Quad object:
    '''
    #
    nP0     = pQds.nP
    nQ0     = pQds.nQ
    zPIDs_0 = pPQids.copy() ; # (nP) All point IDs involved in initial polpulation of Quads
    ...
    zQNam_0 = pQds.QuadNames.copy()  ; # (nQ)
    zQpnt_0 = pQds.MeshVrtcPntIdx ; # (nQ,4)
    ...
    if nQ < nQ0:
        ...
        zQcoor = np.zeros((nP,2))              ; # coordinates of the nP points
        zQpnts = np.zeros((nQ,4), dtype=int)   ; # indices (to be used in xCoor) of the 4 points composing the Quad
        zPQids = np.zeros(nP)                  ; # IDs of points
        zQnam  = np.zeros(nQ,     dtype='U32') ; # Quad names
        ...
        for jQp in range(nQ0):
            ...
            if l_Survived:
                ...
                for jp in range(4):
                    ...
                    (ip,) = np.where( zPQids == iPids ) ; # => index of this point in the frame [0,nP-1]
        ...
        zQcoor = np.zeros((nP,2))
        _,ind2keep,_ = np.intersect1d(vIDs, zPQids, return_indices=True); # retain only indices of `vIDs` that exist in `zPQids`
        zQcoor[:,:] = pCoor[ind2keep,:]
        
    else:
        print('ERROR: nQ > nQ0!!!'); exit(0)

    return zQcoor, zQpnts, zPQids, zQnam, zQIDs
