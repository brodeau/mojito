...
if __name__ == '__main__':
    ...
    xU   = np.zeros((Nt-1,Nb)) + FillValue
    xV   = np.zeros((Nt-1,Nb)) + FillValue

    # Need to have all coordinates and co also centered on buoy time like velocities,
    # because deformation won't use 2 quadrangle files but just one!!!
    ...
    for jid in vIDs:
        ...
        # Computation of velocities before time interpolation:
        zT = np.zeros(Np-1)
        zV = np.zeros(Np-1)
        zU = np.zeros(Np-1)        
        for ip in range(Np-1):
            ii = idx[ip]
            zT[ip] = 0.5*(vtime0[ii+1] + vtime0[ii]) ; # centered time !
            zV[ip] = 1000.*(vy0[ii+1] - vy0[ii])/(vtime0[ii+1] - vtime0[ii]) ; # m/s !!!
            zU[ip] = 1000.*(vx0[ii+1] - vx0[ii])/(vtime0[ii+1] - vtime0[ii]) ; # m/s !!!
