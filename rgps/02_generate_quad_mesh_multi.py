...
if __name__ == '__main__':
    ...
    if l_force_min_scale:
        ...
        if rd_spacing < rd_nom_data:
            print('WARNING: we wont downsample the cloud because `rd_spacing < rd_nom_data` !!!')
            print('         => we use the nominal scale of '+rd_nom_data+' km for `rd_spacing`, without downsampling!')
            rd_spacing = rd_nom_data
    ...
    for jf in range(NbF):
            ...
            if jf==0:
                ...
                while not l_happy:
                    ...
                    if l_force_min_scale:
                        ...
                        NbPss, zCoor, idxlft = mjt.SubSampCloud( rd_ss, xCoor )
                        zIDs  =  vIDs[idxlft]
                        ztime = vtime[idxlft]
                        zPnam = vPnam[idxlft]
                    ...
                    TRIAS = mjt.Triangle( zCoor, xTpnts, xNeighborIDs, zIDs, ztime, zPnam, origin=corgn )
                    ...
                    if l_someQuads:
                        ...
                        QUADS0 = mjt.Quadrangle( xPcoor, xQpnts, vPids, vPtime, vQnam, date=cdate, origin=corgn )
                ...
                if idebug>0 and l_force_min_scale and iplot>0:
                    # Shows the cloud of buoys (with buoys' IDs) on the Cartesian plane (km)
                    # After and before subsampling
                    kk = mjt.ShowTQMesh( xCoor[:,0], xCoor[:,1], cfig=cfdir+'/00_'+cfroot+'_Original.png',
                                         ppntIDs=vIDs, lGeoCoor=False, zoom=rzoom_fig, rangeX=zrx, rangeY=zry )
                    kk = mjt.ShowTQMesh( zCoor[:,0], zCoor[:,1], cfig=cfdir+'/00_'+cfroot+'_SubSamp.png',
                                         ppntIDs=zIDs, lGeoCoor=False, zoom=rzoom_fig, rangeX=zrx, rangeY=zry )

                # Save the triangular mesh info in npz file:
                mjt.SaveClassPolygon( cf_npzT, TRIAS, ctype='T', origin=corgn )
                del TRIAS

                if l_someQuads:
                    # Save the quadrangular mesh info:
                    mjt.SaveClassPolygon( cf_npzQ, QUADS0, ctype='Q', origin=corgn )
                ...
                QUADS = mjt.Quadrangle( xPcoor, xQpnts, vPids, vPtime, vQnam, vQIDs=vQIDs, date=cdate, origin=corgn )
                print(' *** new Quad class updated for current file!\n')

                # Save the quadrangular mesh info:
                mjt.SaveClassPolygon( cf_npzQ, QUADS, ctype='Q', origin=corgn )
        ...
        if jf==0 and iplot>0:
            kk = mjt.ShowTQMesh( zcoor[:,0], zcoor[:,1], cfig=cfdir+'/'+cfroot+'_01_Tmesh.png',
                                 TriMesh=TRI.MeshVrtcPntIdx, lGeoCoor=False, zoom=rzoom_fig, rangeX=zrx, rangeY=zry)

        if l_someQuads and iplot>0:

            if jf==0:
                # Show triangles together with the quadrangles on a map:
                kk = mjt.ShowTQMesh( zcoor[:,0], zcoor[:,1], cfig=cfdir+'/'+cfroot+'_02_Qmesh.png',
            ...
            kk = mjt.ShowTQMesh( QUA.PointXY[:,0], QUA.PointXY[:,1], cfig=cfdir+'/'+cfroot+'_03_Qmesh.png',
