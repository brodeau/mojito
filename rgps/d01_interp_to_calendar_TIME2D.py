...
if __name__ == '__main__':
    ...
    Np0, vtim0, vYkm0, vXkm0, vlat0, vlon0, vBIDs0 = mjt.LoadDataRGPS( cf_in, list_expected_var )
    ...
    for jid in vIDs:
        ...
        (idx,) = np.where( vBIDs0==jid )
        vt   = vtim0[idx] ;            # that's the time axis of this particular buoy [epoch time]
        ...
        ftm = interpolate.interp1d(               vt, vtim0[idx], kind='linear' ) ; # it has to be linear, always, right?
        if   interp_1d==0:
            fGy = interpolate.interp1d(           vt, vlat0[idx], kind='nearest' )
            fCy = interpolate.interp1d(           vt, vYkm0[idx], kind='nearest' )
            fGx = interpolate.interp1d(           vt, vlon0[idx], kind='nearest' )
            fCx = interpolate.interp1d(           vt, vXkm0[idx], kind='nearest' )
        elif interp_1d==1:
            fGy = interpolate.interp1d(           vt, vlat0[idx], kind='linear' )
            fCy = interpolate.interp1d(           vt, vYkm0[idx], kind='linear' )
            fGx = interpolate.interp1d(           vt, vlon0[idx], kind='linear' )
            fCx = interpolate.interp1d(           vt, vXkm0[idx], kind='linear' )
        elif interp_1d==2:
            fGy = interpolate.Akima1DInterpolator(vt, vlat0[idx])
            fCy = interpolate.Akima1DInterpolator(vt, vYkm0[idx])
            fGx = interpolate.Akima1DInterpolator(vt, vlon0[idx])
            fCx = interpolate.Akima1DInterpolator(vt, vXkm0[idx])

        xtim[0:Nt_b,ic] = ftm(vTbin[0:Nt_b,0])
        xlat[0:Nt_b,ic] = fGy(vTbin[0:Nt_b,0])
        xYkm[0:Nt_b,ic] = fCy(vTbin[0:Nt_b,0])
        xlon[0:Nt_b,ic] = fGx(vTbin[0:Nt_b,0])
        xXkm[0:Nt_b,ic] = fCx(vTbin[0:Nt_b,0])
        xmsk[0:Nt_b,ic] = 1
    ...
    kk = mjt.ncSaveCloudBuoys( cf_out, vTbin[:,0], vIDs, xYkm, xXkm, xlat, xlon, mask=xmsk, time2d=xtim,
